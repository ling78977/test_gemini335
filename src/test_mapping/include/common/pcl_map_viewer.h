...
class PCLMapViewer {
    public:
     ...
     PCLMapViewer(const float& leaf_size, bool use_pcl_vis = true)
         : leaf_size_(leaf_size), tmp_cloud_(new sad::PointCloudType), local_map_(new sad::PointCloudType) {
         if (use_pcl_vis) {
      ...
      * @param cloud_world
      */
     void SetPoseAndCloud(const SE3& pose, sad::CloudPtr cloud_world) {
         ...
         *local_map_ += *tmp_cloud_;
         ...
         if (viewer_ != nullptr) {
             ...
             pcl::visualization::PointCloudColorHandlerGenericField<sad::PointType> fieldColor(local_map_, "z");
             viewer_->addPointCloud<sad::PointType>(local_map_, fieldColor, "local_map");
    ...
    private:
     pcl::VoxelGrid<sad::PointType> voxel_filter_;
     pcl::visualization::PCLVisualizer::Ptr viewer_ = nullptr;  // pcl viewer
     float leaf_size_ = 1.0;
     sad::CloudPtr tmp_cloud_;  //
     sad::CloudPtr local_map_;
 };
 #endif